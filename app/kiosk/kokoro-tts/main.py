...
async def synthesize(data: ISynthesize):
    ...
    try:
        voice = CONFIG["speaker"] if data.voice is None else data.voice
        speed = CONFIG["speed"] if data.speed <= 0 else data.speed
        CONFIG["speaker"] = voice
        CONFIG["speed"] = speed
        language = voice[0]
        
        print(f"[MSG]: Current language: {CONFIG['language']}, new language:  {language}")
        if language != CONFIG["language"]:
           print(f"[MSG]: Update language to {language}")
           CONFIG["language"] = language
           KOKOROTTS = KPipeline(lang_code=language)
        
        start_time = time.time()
        generator = KOKOROTTS(
            #text=data.text, voice=CONFIG["speaker"],
            text=data.input, voice=voice,
            speed=speed
        )
        ...
        if data.keep_file:
            ...
            wav_io = io.BytesIO()
            sf.write(wav_io, combined_audio, samplerate=24000, format='WAV')  
            wav_io.seek(0)
